...
@pytest.mark.parametrize("title,content,published",[
    ...
    ('Pepperoni','I love pepperoni',False),
    ('Tallest Skyscrapers','Bhurj Khalifa',True)
])
...
def test_unauthorized_delete_posts(client,test_user,test_posts):
...
def test_delete_post_on_non_exist(authorized_client,test_user,test_posts):

    res = authorized_client.delete(f"/posts/3232")
    assert res.status_code == 404  
...
def test_delete_post_on_non_exist(authorized_client,test_user,test_posts):
    ...
    res = authorized_client.put(f"/posts/3232",json=data)
    assert res.status_code == 404 
